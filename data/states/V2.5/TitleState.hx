...
function startIntro()
{
	...
	var bg:FlxSprite = new FlxSprite().loadGraphic(Paths.image('menus/V2.5/titlestate/bg'));
	...
	logoBl.frames = Paths.getSparrowAtlas('menus/V2.5/logoBumpin');
	...
	gfDance.frames = Paths.getSparrowAtlas('menus/V2.5/titlestate/gfDanceTitle');
	...
	titleText.frames = Paths.getSparrowAtlas('menus/V2.5/titlestate/titleEnter');
	...
	blackScreen = new FlxBackdrop(Paths.image('menus/V2.5/titlestate/scroll'),FlxAxes.X, 0.2, 0.2, true, true);
...
override function update(elapsed:Float)
{
	...
	if (!pressedEnter && !pressedSkip && !transitioning && skippedIntro)
	{
		...
		{
			pressedSkip = true;
			FlxG.switchState(new ModState('V2.5/MainMenuState'));
		}
		if (FlxG.keys.justPressed.A)
		{
			pressedSkip = true;
			FlxG.switchState(new ModState('V2.5/AntiPiracyState'));
		}
	}
}
