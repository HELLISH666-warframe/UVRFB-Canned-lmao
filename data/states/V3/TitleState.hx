...
function startIntro() {
	...
	animScreen.frames = Paths.getSparrowAtlas('menus/V3/titlescreen/trueTitleBgAnimated');
	animScreen.animation.addByPrefix('animate', 'animate', 30, true);
	animScreen.animation.play('animate');
	animScreen.updateHitbox();
	animbarScrt = new FlxBackdrop(Paths.image('menus/V3/titlescreen/trueTitleBarTop'), FlxAxes.X, 0, 0);
	animbarScrb = new FlxBackdrop(Paths.image('menus/V3/titlescreen/trueTitleBarBottom'), FlxAxes.X, 0, 0);
	new FlxTimer().start(0.005, function(tmr:FlxTimer) {
		animbarScrb.x -= 2;
		animbarScrt.x += 2;
		tmr.reset(0.005);
	});
	logoBi = new FlxSprite().loadGraphic(Paths.image('menus/V3/titlescreen/trueTitleBack'));
	logoBl = new FlxSprite().loadGraphic(Paths.image('menus/V3/titlescreen/trueTitleLogo'));
	titleText = new FlxSprite().loadGraphic(Paths.image('menus/V3/titlescreen/trueTitlePlay'));
...
function update(elapsed:Float) {
	...
	if (initialized && !transitioning && skippedIntro && pressedEnter) {
		...
		new FlxTimer().start(1, ()-> {
			FlxG.switchState(new ModState('V3/DesktopState'));
		});
	}
