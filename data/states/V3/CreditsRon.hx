...
var largePortrait:FlxSprite = new FlxSprite(800, 20).loadGraphic(Paths.image("menus/V3/credits/seezee"));
...
function create() {
	...
	bg = CoolUtil.loadAnimatedGraphic(new FlxSprite(340,180),Paths.image('menus/V3/freeplay/classicbgAnimate'));
	...
	for (i in 0...creditJSON.length){
		...
		nameGroup.push(j);
		add(j);
		var e = new FlxSprite().loadGraphic(Paths.image("menus/V3/credits/" + creditJSON[i].name));
...
function changeSelection(e) {
	...
	largePortrait.loadGraphic(Paths.image("menus/V3/credits/" + creditJSON[curSelected].name));
