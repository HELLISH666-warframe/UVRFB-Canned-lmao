...
function postCreate(){
	...
	add(startMenu = new FlxSprite(0, 305).loadGraphic(Paths.image("menus/V3/windowsUi/start menu")));
	for (i in 0...optionArray[0].length) {
		var button = new FlxSprite(25, (723 - startMenu.height) + (34 * i) + (i > 1 ? 120 : 0)+ if(i > 2) 115);
		button.frames = Paths.getSparrowAtlas("menus/V3/windowsUi/win98buttons_plus");
	...
	for (i in 0...optionArray[1].length) {
		...
		sub_button.frames = Paths.getSparrowAtlas("menus/V3/windowsUi/win98buttons_plus");
